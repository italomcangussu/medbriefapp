...
const RegisterWizard: React.FC<RegisterWizardProps> = ({ onComplete, onBackToLogin }) => {
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      setError("Falha ao iniciar cadastro com Google.");
    }
  };
              ...
              <motion.div key="step1" custom={direction} variants={variants} initial="enter" animate="center" exit="exit" transition={springTransition} className="space-y-6 w-full">
                ...
                <div className="w-full">
                  ...
                  >
                    ...
                    <span className="text-[17px] font-semibold text-slate-700">Cadastrar com Google</span>
                  </button>
                </div>
              </motion.div>
            )}
            ...
            {step === 3 && (
              <motion.div key="step3" custom={direction} variants={variants} initial="enter" animate="center" exit="exit" transition={springTransition} className="space-y-6 w-full">
                ...
                <div
                  ...
                  className={`group flex items-center justify-between p-5 rounded-[20px] bg-white border cursor-pointer transition-all active:scale-[0.98] ${formData.acceptedTerms ? 'border-[#007AFF] shadow-sm ring-1 ring-[#007AFF]' : 'border-slate-200 hover:border-slate-300'
                    }`}>
                  ...
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${formData.acceptedTerms ? 'bg-[#007AFF] border-[#007AFF]' : 'border-slate-300'
                    }`}>
